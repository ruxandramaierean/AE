    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Add(int id)
        {
            ...
            if (!shopList.ContainsKey(id))
                ...
                var count = shopList[id];
                shopList[id] = count + 1;
            }

            HttpContext.Session.Set(SessionHelper.ShoppingCart, shopList);

            return RedirectToAction("Details", "Product", new {id = id} );//context.Products.Select(p => new ProductVM().ProdToProdVM(p)).ToList());
        }
        ...
        public IActionResult Remove(int id)
        {
            ...
            if (shopList == null)
                return RedirectToAction("Index", "Product", context.Products.Select(p => new ProductVM().ProdToProdVM(p)).ToList());
